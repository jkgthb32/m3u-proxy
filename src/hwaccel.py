...
class HardwareAccelDetector:
    ...
    def _load_config(self) -> HardwareAccelConfig:
                ...
                with open(hwaccel_file, "r") as f:
                    for line in f:
                        if "=" in line and not line.strip().startswith("#"):
                            ...
                            env_vars[key] = value
                logger.info(f"Loaded hardware acceleration config from {hwaccel_file}")
            except Exception as e:
                logger.warning(f"Failed to load hardware config from file: {e}")

        # Fallback to environment variables
        available = (
            env_vars.get(
                "HW_ACCEL_AVAILABLE", os.getenv("HW_ACCEL_AVAILABLE", "false")
            ).lower()
            == "true"
        )
        accel_type = env_vars.get("HW_ACCEL_TYPE", os.getenv("HW_ACCEL_TYPE", "cpu"))
        device = env_vars.get("HW_ACCEL_DEVICE", os.getenv("HW_ACCEL_DEVICE", ""))
        ...
        config = HardwareAccelConfig(
            available=available, type=accel_type, device=device, ffmpeg_args=ffmpeg_args
        )

        logger.info(f"Hardware acceleration config: {config}")
        return config

    def _generate_ffmpeg_args(
        self, available: bool, accel_type: str, device: str
    ) -> List[str]:
    ...
    def get_transcode_args(
        self, input_codec: Optional[str] = None, output_codec: Optional[str] = None
    ) -> List[str]:
    ...
    def log_capabilities(self):
        ...
        if self.config.available:
            ...
            logger.info("💻 Using CPU-only transcoding (no hardware acceleration)")
