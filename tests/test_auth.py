...
class TestAuthentication:
    ...
    def test_protected_endpoint_with_invalid_token(self, client_with_auth):
        ...
        response = client_with_auth.get(
            "/health", headers={"X-API-Token": "wrong_token"}
        )
    ...
    def test_protected_endpoint_with_valid_token(self, client_with_auth):
        ...
        response = client_with_auth.get(
            "/health", headers={"X-API-Token": "test_token_123"}
        )
    ...
    def test_header_takes_precedence_over_query_param(self, client_with_auth):
        ...
        response = client_with_auth.get(
            "/health?api_token=wrong_token", headers={"X-API-Token": "test_token_123"}
        )
        assert response.status_code != 401
        assert response.status_code != 403
...
class TestAuthenticationIntegration:
    ...
    def test_create_stream_without_auth_fails(self, client_with_auth):
        ...
        response = client_with_auth.post(
            "/streams", json={"url": "http://example.com/stream.m3u8"}
        )
        assert response.status_code == 401
